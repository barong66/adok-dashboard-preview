...
## Информационная иерархия таблиц

- **Единицы измерения в каждом заголовке** — `USD`, `impressions`, `unique users`, `USD / 1k imp.`, `Δ% WoW` и т.п. Под основным названием колонки мелким серым
- **Headline-колонки** (туда падает глаз): Total в Revenue by date/format, eCPM в Traffic Sources, Revenue в Websites — выделены фоном и весом шрифта
- **Цветные точки форматов** в заголовках Revenue by date/format — те же цвета, что в чарте (Popunder фиолетовый, Banner синий и т.д.)
- **Δ% как pill** — `▲ 6.8%` зелёная плашка / `▼ 15.2%` красная плашка / `± 0.3%` нейтральная серая. Быстрее парсится глазом, чем просто цветной текст
- **Right-aligned numbers + tabular-nums** — цифры выравниваются по разрядам
- **Sortable headers с индикатором** ↕ → ↓/↑ при сортировке

## Tooltips везде

- На каждом заголовке колонки — определение метрики и единицы измерения
- На каждой кнопке-табе (Revenue/Impressions/Requests/eCPM, 24h/7d/30d/Custom) — что это значит
- На каждой ⓘ-иконке рядом с названием виджета — назначение виджета и baseline для сравнений
- На каждом sparkline в Trend-колонке — числовое описание тренда («7-day trend: $1.10 → $1.20 (+9.1%)»)
- На Δ%-чипах в Income header — что с чем сравнивается
- На каждом ряду Top movers — полный контекст (name, абс. change, относительное)
